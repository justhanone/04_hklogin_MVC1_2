...
public class AdminDao extends Database{
	...
	public BoardDto getBoard(int seq) {
		...
		try {
			conn=getConnection();
			psmt=conn.prepareStatement(sql);
			psmt.setInt(1, seq); //seq의 타입이 int니까 setInt
			rs=psmt.executeQuery(); //select는 executeQuery(), insert/delete/update=executeUpdate()
			
			while(rs.next()) {
				//java에서 사용할 수 있게 DB의 쿼리를 맵핑해서 가져온다.
				//--> rs.getString=String sql의 query를 executeQuery해서 가져온거니까 그걸 가져와서 넣는 과정
